...
export function AuthForm({ oauthError = null }: { oauthError?: string | null }) {
  ...
  // Seeded with any OAuth failure NextAuth redirected back with, so a failed
  // "Continue with Google" explains itself instead of silently returning here.
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 animate-fade-up">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-gold text-black font-bold rounded-xl hover:bg-gold/90 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-wait"
            >
              {loading
                ? "Please wait..."
                : mode === "signup"
          ...
          <button onClick={handleGoogleLogin} className="w-full mt-8 py-3.5 bg-white/5 border border-gold/30 text-white font-bold rounded-xl hover:bg-gold/10 hover:border-gold/60 transition-all flex items-center justify-center gap-3">
