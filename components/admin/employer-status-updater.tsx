...
import { updateEmployerStatus } from "@/app/actions";
import type { Employer } from "@/types";

const EMPLOYER_STATUSES: Employer["status"][] = [
  "New Inquiry",
  "Contacted",
  "In Progress",
  "Closed Won",
...
export function EmployerStatusUpdater({
  id,
  currentStatus,
}: {
  id: string;
  ...
  function handleSelect(status: Employer["status"]) {
    setOpen(false);
    if (status === currentStatus) return;

    setError(null);
    startTransition(async () => {
      const result = await updateEmployerStatus(id, status);
      if (!result.success) {
